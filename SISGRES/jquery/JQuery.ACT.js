(function ($) {
    ...
    $.extend($.fn, {
        bindKey: function (hotkey, handler)
        {
            ...
            return this.bind('keydown', function (evt)
            {
                ...
                if (evt.keyCode == charCode) result = handler.call(this, evt);
    ...
    $.extend($.fn, {
        tag: function (options) {
            ...
            if (typeof (options) == 'string' && (options == 'select' || options=='remove')) {
                this.data('tag-options')['_'+options].call(this, { target: this }, arguments[1], arguments[2]);
                return;
            }
            ...
            if (typeof (options.autocomplete) != 'undefined') {
                ...
                selector.find('~ .input')[options.autocompleteFunction](autocomplete).focus(function (ev) {
                    ...
                    var tagCount = selector.filter(
                        function() {
                            return $(this).find('~.input:first')[0] == ev.target;
                        }).tag('values').length;
                    if (options.autocomplete.minLength === 0 && options.limit > tagCount) {
            ...
            $('input.input', taglists).bindKey('Esc', function (ev) {
                ...
                if (original.attr('readonly') || original.attr('disabled') || original.hasClass('aspNetDisabled'))
                    return false;
                if ($(ev.target).val() === '') {
                    remove.call(this, ev, $(ev.target).parents('ul.ui-widget-taglist:first').find('li:not(.input):last'));
                    $(this).focus();
                    return false;
                }
            ...
            selector.closest('.ui-widget-taglist').on('click', 'li .ui-icon-close', function (ev)
            {
                var original = $(this).closest('.ui-widget-taglist').find(selector);
                if (original.attr('readonly') || original.attr('disabled') || original.hasClass('aspNetDisabled'))
                    return false;

                remove.call(this, ev, $(this).parent('li'));
                $(this).closest('.ui-widget-taglist').find('li:not(.normalizer):last input.input[type="text"]').focus();
            });

            if (options.add)
                selector.on('tag.add', options.add);
            if (options.remove)
                selector.on('tag.remove', options.remove);
        }
    });
